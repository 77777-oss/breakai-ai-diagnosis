(()=>{
  ...
  function beginPrecisionUtterance(){
    if(!enabled||!mediaStream||precisionBusy||isSpeaking||mediaRecorder?.state==='recording')return;const type=MediaRecorder.isTypeSupported('audio/webm;codecs=opus')?'audio/webm;codecs=opus':'audio/webm';chunks=[];hadSpeech=false;peak=0;voiceStartedAt=performance.now();lastSpeechAt=voiceStartedAt;try{mediaRecorder=new MediaRecorder(mediaStream,{mimeType:type})}catch(_){show('マイクを再接続してください');setState(false);enabled=false;return}
    mediaRecorder.ondataavailable=e=>{if(e.data?.size)chunks.push(e.data)};
    mediaRecorder.onstop=async()=>{const durationMs=Math.max(0,Math.round(performance.now()-voiceStartedAt)),heard=hadSpeech,strong=peak>=.008,blob=new Blob(chunks,{type});if(!enabled)return;if(isSpeaking){setTimeout(()=>{if(enabled&&!isSpeaking)beginPrecisionUtterance()},320);return}if(!heard||!strong){setTimeout(beginPrecisionUtterance,100);return}precisionBusy=true;show('高精度認識中…');try{const text=await transcribePrecision(blob,durationMs);if(text)handleTranscript(text,'precision')}catch(e){show('高精度認識を再接続します');if(String(e.message).includes('UNAUTHORIZED')){voiceToken='';try{sessionStorage.removeItem(TOKEN_KEY)}catch(_){ }}}finally{precisionBusy=false;if(enabled)setTimeout(()=>voiceToken?beginPrecisionUtterance():startWebSpeech(true),100)}};
  ...
  async function startPrecision(fromAuto=false){
    try{mediaStream=mediaStream||await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true},video:false});const AC=window.AudioContext||window.webkitAudioContext;audioContext=audioContext||new AC();try{await Promise.race([audioContext.resume(),new Promise(r=>setTimeout(r,900))])}catch(_){ }if(audioContext.state!=='running'){cleanupMedia();enabled=false;setState(false);show('音声を使うには🎤音声を押してください。');return}analyser=audioContext.createAnalyser();analyser.fftSize=1024;audioContext.createMediaStreamSource(mediaStream).connect(analyser);for(const track of mediaStream.getAudioTracks()){track.onended=()=>{if(enabled){show('マイクが切断されました。再接続します');cleanupMedia();setTimeout(()=>start(false),450)}}}setState(true,'precision');show(fromAuto?'高精度音声を引き継ぎました':'高精度音声を開始しました');startNavigationSpotter();beginPrecisionUtterance()}catch(e){if(e?.name==='NotAllowedError'){enabled=false;setState(false);show('マイク許可が必要です。🎤音声を押してください。');return}show('高精度音声を開始できないためブラウザ認識へ切替');startWebSpeech(true)}
  }
