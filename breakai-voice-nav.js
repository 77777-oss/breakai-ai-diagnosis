(()=>{
  ...
  let persisted=false,shellPersisted=launchVoice,handPersisted=incomingHand,voiceToken=incomingToken,projectId=incomingProject;
  try{persisted=sessionStorage.getItem(KEY)==='1';shellPersisted=shellPersisted||sessionStorage.getItem(SHELL_KEY)==='1';handPersisted=handPersisted||sessionStorage.getItem(HAND_KEY)==='1';voiceToken=voiceToken||sessionStorage.getItem(TOKEN_KEY)||'';projectId=projectId||sessionStorage.getItem(PROJECT_KEY)||''}catch(_){ }
  if(!launchVoice&&!persisted&&!shellPersisted)return;
  ...
  function handleConnectedAction(action){if(!action)return;if(action.requires_confirmation){pushChat('system','この操作は外部への影響があるため、司令塔のHuman Gateで確認が必要です。');return}if(action.type==='workspace_back'){if(history.length>1)history.back();else toCommand();return}if(action.type==='open_workspace'&&action.project_id&&action.project_id!==projectId){toCommand(`${action.project_id}を開いて`);return}}
  async function speakConnected(text){if(!voiceToken||!text||isSpeaking)return;isSpeaking=true;stopNavigationSpotter();try{if(mediaRecorder?.state==='recording'){hadSpeech=false;try{mediaRecorder.stop()}catch(_){ }}const r=await fetch(COMMAND_ORIGIN+'/api/connected-speech',{method:'POST',headers:{'Content-Type':'application/json','X-BreakAI-Voice-Token':voiceToken,'X-BreakAI-Project':projectId},body:JSON.stringify({text:String(text).slice(0,900)}),mode:'cors',cache:'no-store'});if(!r.ok)return;const blob=await r.blob();replyAudio?.pause();replyAudio=new Audio(URL.createObjectURL(blob));await replyAudio.play();await new Promise(res=>{replyAudio.onended=res;replyAudio.onerror=res})}catch(_){ }finally{isSpeaking=false;if(enabled&&voiceToken){setTimeout(startNavigationSpotter,320);if(mediaStream)setTimeout(beginPrecisionUtterance,320)}}}
  ...
  document.addEventListener('visibilitychange',()=>{if(document.hidden){if(mediaRecorder?.state==='recording'){hadSpeech=false;try{mediaRecorder.stop()}catch(_){ }}}else if(enabled&&voiceToken&&mediaStream&&!isSpeaking){setTimeout(beginPrecisionUtterance,180)}});
