(()=>{
  ...
  const CUSTOMER_TOKEN_KEY='breakai.customer.entitlementToken';
  const CUSTOMER_PROJECT_KEY='breakai.customer.project';
  ...
  const hostProjects={'ses.breakai-work.com':'ses','verify.breakai-labs.co.jp':'verify','radar.breakai-labs.co.jp':'radar','ai-kano.ai':'aikano','platform.breakai-work.com':'platform','77777-oss.github.io':location.pathname.includes('ai-search')?'geo_aeo':'auto_revenue','breakai-labs.co.jp':'website'};
  const inferredProject=hostProjects[location.hostname]||'';
  const incomingAccessToken=String(fragment.get('breakai_access_token')||'').trim();
  const incomingCustomerProject=String(fragment.get('breakai_project')||'').trim();
  ...
  const incomingProject=String(q.get('breakai_voice_project')||fragment.get('breakai_voice_project')||incomingCustomerProject||inferredProject).trim();
  const incomingHand=q.get('breakai_hand')==='1'||fragment.get('breakai_hand')==='1';
  if(launchVoice){
    try{sessionStorage.setItem(KEY,'1');sessionStorage.setItem(SHELL_KEY,'1');if(incomingToken)sessionStorage.setItem(TOKEN_KEY,incomingToken);if(incomingProject)sessionStorage.setItem(PROJECT_KEY,incomingProject);if(incomingHand)sessionStorage.setItem(HAND_KEY,'1')}catch(_){ }
  ...
  if(incomingAccessToken){
    try{localStorage.setItem(CUSTOMER_TOKEN_KEY,incomingAccessToken);if(incomingProject)localStorage.setItem(CUSTOMER_PROJECT_KEY,incomingProject)}catch(_){ }
    fragment.delete('breakai_access_token');fragment.delete('breakai_project');
    const cleanHash=fragment.toString()?`#${fragment}`:'';
    const clean=location.pathname+(q.toString()?`?${q}`:'')+cleanHash;
    try{history.replaceState(history.state,'',clean)}catch(_){ }
  }
  let persisted=false,shellPersisted=launchVoice,handPersisted=incomingHand,voiceToken=incomingToken,projectId=incomingProject,customerToken=incomingAccessToken;
  try{persisted=sessionStorage.getItem(KEY)==='1';shellPersisted=shellPersisted||sessionStorage.getItem(SHELL_KEY)==='1';handPersisted=handPersisted||sessionStorage.getItem(HAND_KEY)==='1';voiceToken=voiceToken||sessionStorage.getItem(TOKEN_KEY)||'';projectId=projectId||sessionStorage.getItem(PROJECT_KEY)||localStorage.getItem(CUSTOMER_PROJECT_KEY)||inferredProject;customerToken=customerToken||localStorage.getItem(CUSTOMER_TOKEN_KEY)||''}catch(_){ }
  const customerMode=!!customerToken&&!launchVoice&&!persisted&&!shellPersisted;
  if(!launchVoice&&!persisted&&!shellPersisted&&!customerToken)return;

  const host=document.createElement('div');host.id='breakai-universal-nav';
  host.style.cssText='position:fixed;left:max(10px,env(safe-area-inset-left));bottom:max(10px,env(safe-area-inset-bottom));z-index:2147483647;pointer-events:auto'+(customerMode?';display:none':'');
  ...
  let entitlementFeatures={manual_ui:true,ai_chat:!customerMode,voice:!customerMode,hand:!customerMode,agent_assist:false},entitlementResolved=!customerMode,entitlementPlan='';
  const connectedAuthHeaders=(extra={})=>{const h={...extra,'X-BreakAI-Project':projectId};if(voiceToken)h['X-BreakAI-Voice-Token']=voiceToken;if(customerToken)h['X-BreakAI-Entitlement']=customerToken;return h};
  function applyCustomerEntitlement(d){const next=d?.features||{};entitlementFeatures=next;entitlementPlan=d?.plan_label||d?.plan||'';entitlementResolved=true;$('#home').style.display='none';chatButton.style.display=next.ai_chat?'':'none';mic.style.display=next.voice?'':'none';handButton.style.display=next.hand?'':'none';chatButton.title=next.agent_assist?'AI Agent Assist':'この画面でAIと会話';if(!next.ai_chat&&conversation.classList.contains('open'))closeChat();if(!next.voice&&enabled)stop();if(!next.hand&&handEnabled)stopHand(false);if(!next.ai_chat&&!next.voice&&!next.hand){host.style.display='none';return false}status.textContent=entitlementPlan?`契約: ${entitlementPlan}${next.agent_assist?' / Agent Assist':''}`:'利用可能';host.style.display='';return true}
  async function resolveCustomerEntitlement(){
    if(!customerMode)return true;
    try{const r=await fetch(COMMAND_ORIGIN+'/api/entitlement/resolve',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:customerToken,project_id:projectId}),mode:'cors',cache:'no-store'}),d=await r.json().catch(()=>({}));if(!r.ok||!d.authorized)throw new Error('NOT_AUTHORIZED');return applyCustomerEntitlement(d)}catch(_){try{localStorage.removeItem(CUSTOMER_TOKEN_KEY);localStorage.removeItem(CUSTOMER_PROJECT_KEY)}catch(__){ }customerToken='';entitlementResolved=false;try{stop();stopHand(false);closeChat()}catch(__){ }host.remove();return false}}
  ...
  const toCommand=(query='')=>{
    if(customerMode){show('この契約では他システムへの移動はできません');return}
  ...
  function openChat(){if(customerMode&&(!entitlementResolved||!entitlementFeatures.ai_chat)){show('現在のプランではAI会話は利用できません');return}conversation.classList.add('open');chatButton.dataset.live='1';$('#chat-title').textContent=(projectNames[projectId]||'接続システム')+' AI';renderChat();setTimeout(()=>chatInput.focus(),0)}
  ...
  function handleConnectedAction(action){if(!action)return;if(action.requires_confirmation){pushChat('system','この操作は現実世界に影響するため、明示許可済みの自動化でない限りFounder本人の最終確認が必要です。');return}if(action.type==='workspace_back'){if(history.length>1)history.back();else toCommand();return}if(action.type==='open_workspace'&&action.project_id&&action.project_id!==projectId){toCommand(`${action.project_id}を開いて`);return}}
  async function speakConnected(text){if(!(voiceToken||customerToken)||!text||isSpeaking)return;isSpeaking=true;stopNavigationSpotter();try{if(mediaRecorder?.state==='recording'){hadSpeech=false;try{mediaRecorder.stop()}catch(_){ }}const r=await fetch(COMMAND_ORIGIN+'/api/connected-speech',{method:'POST',headers:connectedAuthHeaders({'Content-Type':'application/json'}),body:JSON.stringify({text:String(text).slice(0,900)}),mode:'cors',cache:'no-store'});if(!r.ok)return;const blob=await r.blob();replyAudio?.pause();replyAudio=new Audio(URL.createObjectURL(blob));await replyAudio.play();await new Promise(res=>{replyAudio.onended=res;replyAudio.onerror=res})}catch(_){ }finally{isSpeaking=false;if(enabled&&(voiceToken||customerToken)){setTimeout(startNavigationSpotter,320);if(mediaStream)setTimeout(beginPrecisionUtterance,320)}}}
  function customerPageSnapshot(){if(!customerMode||!entitlementFeatures.agent_assist)return'';try{const raw=String(document.body?.innerText||'').replace(/\s+/g,' ').trim();return raw.slice(0,12000)}catch(_){return''}}
  async function connectedAsk(raw){const text=String(raw||'').trim();if(!text||chatBusy)return false;if(customerMode&&(!entitlementResolved||!entitlementFeatures.ai_chat)){show('現在のプランではAI会話は利用できません');return true}if(!(voiceToken||customerToken)||!projectId){toCommand(text);return true}openChat();const prior=chatHistory.slice(-10);pushChat('user',text);chatBusy=true;show(entitlementFeatures.agent_assist?'Agent Assistが画面を確認中…':'この画面でAIが確認中…');const wait=document.createElement('div');wait.className='msg ai thinking';wait.textContent=entitlementFeatures.agent_assist?'画面を確認して進め方を考えています…':'確認しています…';chatLog.append(wait);chatLog.scrollTop=chatLog.scrollHeight;try{const r=await fetch(COMMAND_ORIGIN+'/api/connected-assist',{method:'POST',headers:connectedAuthHeaders({'Content-Type':'application/json'}),body:JSON.stringify({message:text,history:prior,page:{path:location.pathname+location.search,title:document.title,visible_text:customerPageSnapshot()}}),mode:'cors',cache:'no-store'});const d=await r.json().catch(()=>({}));wait.remove();if(!r.ok)throw new Error(d.detail||d.error||'CONNECTED_ASSIST_FAILED');lastSources=Array.isArray(d.sources)?d.sources:[];pushChat('ai',d.reply||'確認結果を表示しました。',lastSources);handleConnectedAction(d.action);show(entitlementFeatures.agent_assist?'Agent Assistの結果を表示しました':'回答をこの画面に表示しました');void speakConnected(d.speech_text||d.reply||'')}catch(e){wait.remove();pushChat('system',`確認できませんでした: ${String(e.message||e).slice(0,120)}`);show('AI接続を確認してください')}finally{chatBusy=false}return true}
  ...
  const normalize=t=>String(t||'').replace(/[\s　]+/g,'').replace(/指令塔|指令等|指令棟|司令棟|司令等|司令東|司令党|司令と|司令塔ー|しれいとう|しれい塔|司令とう/g,'司令塔').replace(/もどって/g,'戻って').replace(/もどる/g,'戻る').replace(/ファイナンシャルシステム|ファイナンスシステム|金融システム/g,'FinancialAI').replace(/スカター|スカウタ/g,'SCOUTER');
  ...
  const dangerous=/削除|消去|送信|決済|支払|購入|発注|注文|本番反映|デプロイ|公開|投稿|保存|登録|実行|開始|停止|承認|送金|振込|出金|売買|オン|オフ/i;
  ...
  let handEnabled=false,handStarting=false,handStream=null,handLandmarker=null,handPointerX=innerWidth/2,handPointerY=innerHeight/2,pinchDown=false,lastPinchAt=0,lastPalmX=null,lastPalmAt=0,lastSwipeAt=0,handTimer=null,lastHandDetectAt=0,lastHandSeenAt=0;
  ...
  function pinchRatio(lm){return distance(lm?.[4],lm?.[8])/Math.max(.055,distance(lm?.[5],lm?.[17]))}
  function virtualCamera(label){return /virtual|仮想|obs|manycam|snap camera|nvidia broadcast|camo|epoccam|mmhmm/i.test(String(label||''))}
  async function acquireHandStream(){let devices=[];try{devices=(await navigator.mediaDevices.enumerateDevices()).filter(d=>d.kind==='videoinput');if(devices.length&&devices.every(d=>!String(d.label||'').trim())){const probe=await navigator.mediaDevices.getUserMedia({video:true,audio:false});probe.getTracks().forEach(t=>t.stop());devices=(await navigator.mediaDevices.enumerateDevices()).filter(d=>d.kind==='videoinput')}}catch(_){}
    const preferred=devices.filter(d=>!virtualCamera(d.label)),fallback=devices.filter(d=>virtualCamera(d.label)),attempts=[];
    for(const d of preferred)if(d.deviceId)attempts.push({video:{deviceId:{exact:d.deviceId},facingMode:{ideal:'user'},width:{ideal:960},height:{ideal:540},frameRate:{ideal:30,max:30}},audio:false});
    attempts.push({video:{facingMode:{ideal:'user'},width:{ideal:960},height:{ideal:540},frameRate:{ideal:30,max:30}},audio:false});
    for(const d of fallback)if(d.deviceId)attempts.push({video:{deviceId:{exact:d.deviceId},width:{ideal:960},height:{ideal:540},frameRate:{ideal:30,max:30}},audio:false});
    let last=null;for(const c of attempts){try{return await navigator.mediaDevices.getUserMedia(c)}catch(e){last=e}}throw last||new Error('CAMERA_NOT_AVAILABLE')}
  ...
  function scheduleHandFrame(delay=45){clearTimeout(handTimer);if(handEnabled&&handStream&&handLandmarker)handTimer=setTimeout(handFrame,delay)}
  function stopHand(persist=true){handEnabled=false;handStarting=false;clearTimeout(handTimer);handTimer=null;handStream?.getTracks().forEach(t=>t.stop());handStream=null;handLandmarker=null;handVideo.srcObject=null;handPointer.className='';lastPalmX=null;pinchDown=false;lastHandSeenAt=0;if(persist){try{sessionStorage.setItem(HAND_KEY,'0')}catch(_){ }}setHandState(false,'手操作を停止しました')}
  function handFrame(){
    if(!handEnabled||!handStream||!handLandmarker)return;
    const now=performance.now();if(now-lastHandDetectAt<65){scheduleHandFrame(35);return}lastHandDetectAt=now;
    if(handVideo.readyState<2){scheduleHandFrame(60);return}
    let result=null;try{result=handLandmarker.detectForVideo(handVideo,now)}catch(_){scheduleHandFrame(80);return}
    const lm=result?.landmarks?.[0];if(lm?.[8]){lastHandSeenAt=now;const tx=(1-lm[8].x)*innerWidth,ty=lm[8].y*innerHeight;handPointerX+=(tx-handPointerX)*.34;handPointerY+=(ty-handPointerY)*.34;const hit=safeHandTarget(handPointerX,handPointerY);handPointer.className='live'+(hit?.blocked?' blocked':'');handPointer.style.left=`${handPointerX}px`;handPointer.style.top=`${handPointerY}px`;const tip=distance(lm[4],lm[8]),ratio=pinchRatio(lm),pinch=pinchDown?(ratio<.68||tip<.080):(ratio<.52||tip<.070);if(pinch&&!pinchDown&&now-lastPinchAt>650){lastPinchAt=now;handPointer.classList.add('pinch');if(hit?.blocked){show(`「${hit.label||'重要操作'}」は手操作で直接実行しません。手動で確認してください。`)}else if(hit?.el){hit.el.click();show(`${hit.label||'項目'}を選択しました`)} }pinchDown=pinch;
      const palmX=(1-(lm[9]?.x??lm[0]?.x??lm[8].x))*innerWidth,open=handOpen(lm);if(open){if(lastPalmX!=null&&now-lastPalmAt<360&&now-lastSwipeAt>900){const dx=palmX-lastPalmX;if(Math.abs(dx)>Math.max(165,innerWidth*.12)){lastSwipeAt=now;lastPalmX=palmX;lastPalmAt=now;if(dx>0){show('手のスワイプ：戻ります');history.back()}else{show('手のスワイプ：進みます');history.forward()}}}else{lastPalmX=palmX;lastPalmAt=now}}else{lastPalmX=null}
    }else{if(now-lastHandSeenAt>HAND_GRACE_MS){handPointer.className='';pinchDown=false;lastPalmX=null}}
    scheduleHandFrame(45)
  }
  async function startHand(fromAuto=false){if(customerMode&&(!entitlementResolved||!entitlementFeatures.hand)){show('現在のプランでは手操作は利用できません');return}if(handEnabled||handStarting)return;handStarting=true;try{const stream=await acquireHandStream();const vision=await import(MP_BASE+'/vision_bundle.mjs');const files=await vision.FilesetResolver.forVisionTasks(MP_BASE+'/wasm');const landmarker=await vision.HandLandmarker.createFromOptions(files,{baseOptions:{modelAssetPath:MP_BASE+'/models/hand_landmarker.task'},runningMode:'VIDEO',numHands:1,minHandDetectionConfidence:.15,minHandPresenceConfidence:.15,minTrackingConfidence:.15});handStream=stream;handLandmarker=landmarker;handVideo.srcObject=stream;await handVideo.play();handPointerX=innerWidth/2;handPointerY=innerHeight/2;lastHandDetectAt=0;lastHandSeenAt=0;handEnabled=true;try{sessionStorage.setItem(HAND_KEY,'1')}catch(_){ }setHandState(true,fromAuto?'手操作を引き継ぎました。指先で移動、つまむ動作で選択できます。':'手操作を開始しました。指先で移動、つまむ動作で選択できます。');scheduleHandFrame(20)}catch(e){handStream?.getTracks().forEach(t=>t.stop());handStream=null;handLandmarker=null;setHandState(false,e?.name==='NotAllowedError'?'カメラ許可が必要です。🖐手操作を押してください。':'手操作を開始できませんでした。手動操作は引き続き使えます。')}finally{handStarting=false}}
  ...
  async function transcribePrecision(blob,durationMs){
    const endpoint=COMMAND_ORIGIN+'/api/connected-transcribe',controller=new AbortController(),timer=setTimeout(()=>controller.abort(),15000);
    try{const r=await fetch(endpoint,{method:'POST',headers:connectedAuthHeaders({'Content-Type':blob.type||'audio/webm','X-BreakAI-Audio-Ms':String(durationMs)}),body:blob,mode:'cors',cache:'no-store',signal:controller.signal});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||'TRANSCRIBE_FAILED');return String(d.text||'').trim()}finally{clearTimeout(timer)}
  }
  function monitorPrecision(){
    if(!enabled||!mediaRecorder||mediaRecorder.state!=='recording'||!analyser)return;if(isSpeaking){requestAnimationFrame(monitorPrecision);return}const a=new Uint8Array(analyser.fftSize);analyser.getByteTimeDomainData(a);let sum=0;for(const v of a){const x=(v-128)/128;sum+=x*x}const rms=Math.sqrt(sum/a.length),now=performance.now();peak=Math.max(peak,rms);if(rms>.012){hadSpeech=true;lastSpeechAt=now}const e=now-voiceStartedAt;show(hadSpeech?'聞き取り中…':'話しかけてください',false);if((hadSpeech&&e>650&&now-lastSpeechAt>750)||e>6500||(!hadSpeech&&e>3800)){try{mediaRecorder.stop()}catch(_){ }return}requestAnimationFrame(monitorPrecision)
  }
  function preferredAudioMime(){for(const type of ['audio/webm;codecs=opus','audio/webm','audio/mp4','audio/mp4;codecs=mp4a.40.2']){try{if(MediaRecorder.isTypeSupported(type))return type}catch(_){ }}return ''}
  function beginPrecisionUtterance(){
    if(!enabled||!mediaStream||precisionBusy||isSpeaking||mediaRecorder?.state==='recording')return;const type=preferredAudioMime();chunks=[];hadSpeech=false;peak=0;voiceStartedAt=performance.now();lastSpeechAt=voiceStartedAt;try{mediaRecorder=type?new MediaRecorder(mediaStream,{mimeType:type}):new MediaRecorder(mediaStream)}catch(_){show('高精度録音に未対応のためブラウザ音声認識へ切替');cleanupMedia();setState(false);setTimeout(()=>startWebSpeech(true),80);return}
    mediaRecorder.ondataavailable=e=>{if(e.data?.size)chunks.push(e.data)};
    mediaRecorder.onstop=async()=>{const durationMs=Math.max(0,Math.round(performance.now()-voiceStartedAt)),heard=hadSpeech,strong=peak>=.008,mediaType=mediaRecorder?.mimeType||type||'audio/webm',blob=new Blob(chunks,{type:mediaType});if(!enabled)return;if(isSpeaking){setTimeout(()=>{if(enabled&&!isSpeaking)beginPrecisionUtterance()},320);return}if(!heard||!strong){setTimeout(beginPrecisionUtterance,100);return}precisionBusy=true;show('高精度認識中…');try{const text=await transcribePrecision(blob,durationMs);if(text)handleTranscript(text,'precision')}catch(e){show('高精度認識を再接続します');if(String(e.message).includes('UNAUTHORIZED')){if(customerMode){customerToken='';try{localStorage.removeItem(CUSTOMER_TOKEN_KEY)}catch(_){ }}else{voiceToken='';try{sessionStorage.removeItem(TOKEN_KEY)}catch(_){ }}}}finally{precisionBusy=false;if(enabled)setTimeout(()=>(voiceToken||customerToken)?beginPrecisionUtterance():startWebSpeech(true),100)}};
  ...
  async function startPrecision(fromAuto=false){
    try{mediaStream=mediaStream||await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true},video:false});if(!enabled){cleanupMedia();return}const AC=window.AudioContext||window.webkitAudioContext;audioContext=audioContext||new AC();try{await Promise.race([audioContext.resume(),new Promise(r=>setTimeout(r,900))])}catch(_){ }if(!enabled){cleanupMedia();return}if(audioContext.state!=='running'){cleanupMedia();show('高精度音声を開始できないためブラウザ認識へ切替');startWebSpeech(true);return}analyser=audioContext.createAnalyser();analyser.fftSize=1024;audioContext.createMediaStreamSource(mediaStream).connect(analyser);for(const track of mediaStream.getAudioTracks()){track.onended=()=>{if(enabled){show('マイクが切断されました。再接続します');cleanupMedia();setTimeout(()=>start(false),450)}}}setState(true,'precision');show(fromAuto?'高精度音声を引き継ぎました':'高精度音声を開始しました');startNavigationSpotter();beginPrecisionUtterance()}catch(e){if(e?.name==='NotAllowedError'){enabled=false;setState(false);show('マイク許可が必要です。🎤音声を押してください。');return}show('高精度音声を開始できないためブラウザ認識へ切替');startWebSpeech(true)}
  }
  function startWebSpeech(fromAuto=false){
    ...
    if(!rec){rec=new SR();rec.lang='ja-JP';rec.continuous=true;rec.interimResults=true;rec.maxAlternatives=5;rec.onresult=e=>{for(let i=e.resultIndex;i<e.results.length;i++){const r=e.results[i];if(!r.isFinal)continue;let best=r[0]?.transcript||'';for(let j=0;j<r.length;j++){const t=r[j]?.transcript||'';if(/司令塔|戻|進|開|スクロール|調べ|検索|Financial|ファイナンシャル|スカウター|MATCH|マッチ/i.test(t)){best=t;break}}handleTranscript(best,'browser')}};rec.onerror=e=>{if(e.error==='not-allowed'||e.error==='service-not-allowed'){enabled=false;setState(false);show('マイク許可が必要です。🎤音声を押してください。');return}if(e.error!=='no-speech')show(`音声再接続: ${e.error}`)};rec.onend=()=>{if(enabled){clearTimeout(restartTimer);restartTimer=setTimeout(()=>{try{rec.start()}catch(_){ }},150)}}}
    setState(true,'browser');show(fromAuto?'音声操作を引き継ぎました':'音声操作を開始しました');try{rec.start()}catch(_){ }
  }
  function start(fromAuto=false){if(customerMode&&(!entitlementResolved||!entitlementFeatures.voice)){show('現在のプランでは音声操作は利用できません');return}enabled=true;if(!customerMode){try{sessionStorage.setItem(KEY,'1')}catch(_){ }}if((voiceToken||customerToken)&&projectId&&navigator.mediaDevices?.getUserMedia&&window.MediaRecorder){void startPrecision(fromAuto);return}startWebSpeech(fromAuto)}
  mic.onclick=()=>enabled?stop():start(false);
  document.addEventListener('visibilitychange',()=>{if(document.hidden){if(mediaRecorder?.state==='recording'){hadSpeech=false;try{mediaRecorder.stop()}catch(_){ }}}else if(enabled&&(voiceToken||customerToken)&&mediaStream&&!isSpeaking){setTimeout(beginPrecisionUtterance,180)}});
  ...
  if(customerMode){void resolveCustomerEntitlement();window.addEventListener('focus',()=>void resolveCustomerEntitlement());document.addEventListener('visibilitychange',()=>{if(!document.hidden)void resolveCustomerEntitlement()});setInterval(()=>{if(!document.hidden&&customerToken)void resolveCustomerEntitlement()},60000)}
