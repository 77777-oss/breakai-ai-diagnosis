...
const state={step:1,industry:'',task:'',pain:'other',minutes:0,weekly:0,rate:.5};
const industries=['建設・施工','不動産','物流','製造','介護','飲食・店舗','士業','EC・小売','人材・採用','事務・バックオフィス','その他'];
const tasks=[
 ['見積もり・提案書',.60,'estimate'],['問い合わせ対応',.55,'inquiry'],['メールの仕分け',.60,'email'],
 ['報告書・資料作成',.55,'report'],['データ入力・転記',.65,'data'],['集計・チェック',.45,'data'],
 ['日報・記録',.55,'report'],['検索・情報収集',.50,'content'],['予約・日程調整',.50,'other'],['その他',.45,'other']
];
...
function step2(){progress(2);resetArea();$('#question').textContent='一番減らしたい仕事は？';$('#hint').textContent='今いちばん面倒なものを1つだけ選んでください。';tasks.forEach(([x,r,pain])=>choice.append(button(x,()=>{state.rate=r;state.pain=pain;if(x==='その他')showCustom('例：請求書の確認',v=>{state.task=v;step3();});else{state.task=x;step3();}})));}
...
function taggedUrl(url){if(!url)return '';const sep=url.includes('?')?'&':'?';return `${url}${sep}client_reference_id=arf_${encodeURIComponent(state.pain)}`;}
function showResult(){const before=state.minutes*state.weekly*4/60;const scenarioRate=.30;const saved=before*scenarioRate;const after=Math.max(0,before-saved);$('#resultTask').textContent=state.task;$('#savedHours').textContent=`約${saved.toFixed(1)}時間 / 月`;$('#beforeHours').textContent=`約${before.toFixed(1)}時間/月`;$('#afterHours').textContent=`約${after.toFixed(1)}時間/月`;$('#yearHours').textContent=`約${Math.round(saved*12)}時間/年`;$('#nextAction').textContent=`「${state.task}」をまず1つだけ小さく試し、実際に時間が減るか確認します。`;$('#diag').classList.add('hidden');$('#result').classList.remove('hidden');const c=window.BREAKAI_CONFIG||{};wire('#intake',taggedUrl(c.freeIntakeUrl));wire('#starter',taggedUrl(c.starterUrl));wire('#report',taggedUrl(c.reportUrl));$('#result').scrollIntoView({behavior:'smooth'});}
function wire(id,url){const a=$(id);if(!a)return;if(url){a.href=url;a.target='_blank';a.rel='noopener';}}
$('#restart').onclick=()=>{Object.assign(state,{step:1,industry:'',task:'',pain:'other',minutes:0,weekly:0,rate:.5});$('#result').classList.add('hidden');$('#diag').classList.remove('hidden');step1();$('#diag').scrollIntoView({behavior:'smooth'});};
const c=window.BREAKAI_CONFIG||{};if(c.live&&c.freeIntakeUrl){$('#liveCtas').classList.remove('hidden');wire('#intake',c.freeIntakeUrl);if(c.paidLive&&c.starterUrl&&c.reportUrl){wire('#starter',c.starterUrl);wire('#report',c.reportUrl);}else{const offers=document.querySelector('.offers');if(offers)offers.classList.add('hidden');}}step1();
